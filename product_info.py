...
def get_product_list(product_name):
    ...
    try:
        ...
        for item in product_items:
            ...
            brand = item.find(class_='brand').string if item.find(class_='brand') is not None else ''
            brand_url = item.find(class_='brand').get('href') if item.find(class_='brand') is not None else ''
...
def get_nutrition(product_url, product_info):
    ...
    if soup_generic is not None:
        ...
        product_info['carbohydrate'] = nutrition_facts[5].string
        product_info['protein'] = nutrition_facts[7].string

    else:
        pass

    product_info['manufacturer'] = soup.find(class_='manufacturer').find_all('a')[0].string if soup.find(
        class_='manufacturer') is not None else ''
